...
function calcFreeFallTime(height) {
    const ACCELERATION_OF_GRAVITY = 9.80665; //Acceleration of gravity
    const freeFallTime = Math.sqrt((2 * height) / ACCELERATION_OF_GRAVITY);
    return freeFallTime;
}
// Для высоты 100м должно вывести примерно 4.516сек
console.log(
    `Время свободного падения с высоты 100 метров: ${calcFreeFallTime(100)} сек`
);
/**
...
function calcTriangleAreaFromEdges(a, b, c) {
    const triangleHalfPerimetr = (a + b + c) / 2;
    const triangleArea = Math.sqrt(
        triangleHalfPerimetr * (triangleHalfPerimetr - a) * (triangleHalfPerimetr - b) * (triangleHalfPerimetr - c)
    );
    return triangleArea;
}
